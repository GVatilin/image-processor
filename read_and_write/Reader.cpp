...
read_and_write::Reader::Reader(const std::string& filename) {
    path_ = filename;
}

Image read_and_write::Reader::Read() {
    ...
    try {
        ...
        throw std::invalid_argument("Error reading file: " + static_cast<std::string>(e.what()));
    }
};

size_t read_and_write::Reader::BytesToNum(unsigned char* bytes) {  // NOLINT
    size_t num = *bytes;
...
size_t read_and_write::GetPaddedSize(size_t width) {
